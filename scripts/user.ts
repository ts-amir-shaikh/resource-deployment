...
async function main() {
  ...
  if (has('list') || process.argv.length <= 2) {
    ...
    for (const r of rows) {
      ...
      console.log(`  ${String(r.username).padEnd(16)} ${String(r.role).padEnd(12)} ${r.name}${status}`);
    }
    console.log('');
    client.close();
    return;
  ...
  if (has('create')) {
    ...
    console.log(`\nCreating ${role} account "${username}" (${name}) in ${target}.`);
    const password = await askPasswordTwice();
    const salt = newSalt();
    const hash = await hashPassword(password, salt);
    await client.execute({
      sql: 'insert into users (name, username, password_hash, password_salt, role, active) values (?, ?, ?, ?, ?, 1)',
      args: [name, username, hash, salt, role],
    });
  ...
  if (has('reset')) {
    ...
    console.error('\nNothing to do. Pass one of --create, --reset, --role, --disable, --enable, --list.');
    process.exit(1);
  }
